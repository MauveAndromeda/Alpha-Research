#!/usr/bin/env python3
"""
v4.1 INSTITUTIONAL-GRADE ALPHA SYSTEM

Fixes for institutional audit compliance:
1. Market impact model (Almgren-Chriss simplified)
2. Survivorship bias disclosure and adjustment
3. Proper n_trials for Deflated Sharpe (reflects actual complexity)
4. T+1 rebalancing (conservative)
5. Comprehensive limitations disclosure
6. Stricter p-value requirements

This version is designed to withstand peer review and institutional scrutiny.

Usage:
    python scripts/run_alpha_v4_institutional.py --start 2024-01-01 --end 2025-01-20
"""
...
def compute_market_impact(
    ...
    Components:
    1. Temporary impact (price moves during execution)
...
class MultiFactorModel:
    ...
    def compute_all_factors(self) -> Dict[str, pd.Series]:
        ...
        # Momentum (12-1 month) - Skip most recent month
        if n >= 252:
...
EXPERTS = {
    'momentum_expert': {'role': 'Momentum Specialist', 'prompt': "You are a momentum trading expert."},
    'risk_expert': {'role': 'Risk Manager', 'prompt': "You are a risk manager."},
    'macro_expert': {'role': 'Macro Analyst', 'prompt': "You are a macro analyst."},
    'quant_expert': {'role': 'Quantitative Analyst', 'prompt': "You are a quant."},
    'contrarian_expert': {'role': 'Contrarian', 'prompt': "You are a contrarian."},
}
...
def run_expert_debate(client: LLMClient, market_state: Dict, proposed_mode: str) -> Dict:
    ...
    state_summary = f"""Market: {market_state.get('regime', 'neutral')}, Vol: {market_state.get('vol_regime', 'normal')}, Mode: {proposed_mode}"""
    ...
    for expert_id in EXPERTS.keys():
        ...
        prompt = f"""{state_summary}\nAs {expert['role']}, output JSON: {{"verdict": "APPROVE"/"REJECT"/"CAUTION", "reason": "brief"}}"""
    ...
    return {'verdict': final_verdict, 'confidence': max(votes.values()) / 5, 'votes': votes, 'opinions': opinions}
...
# =============================================================================
# MODE CONFIGURATIONS (Conservative for institutional use)
# =============================================================================

MODE_CONFIGS = {
    'turbo': {
        'factor_weights': {'momentum': 0.45, 'trend': 0.20, 'quality': 0.20, 'low_vol': 0.10, 'reversal': 0.05},
        'leverage_range': (1.2, 2.0),  # More conservative
        'vol_target': 0.18,
        'top_n': 10,
    },
    'balanced': {
...
MODE_SYSTEM = """You are a portfolio mode selector. Output JSON: {"mode": "TURBO"/"BALANCED"/"DEFENSE", "confidence": 0.0-1.0}"""
...
def select_mode_with_llm(client: LLMClient, market_state: Dict) -> Tuple[str, float]:
    ...
    prompt = f"Market: {market_state['regime']}, Vol: {market_state['vol_regime']}. Select mode:"
...
def run_backtest(
    ...
    pending_weights = None  # For T+1 rebalancing
    ...
    for i, date in enumerate(dates):
        ...
        # Trade cost from previous day's rebalance decision
        ...
        if should_update:
            ...
            for leader in causal_leaders:
                if leader in scores.index:
                    scores[leader] *= 1.15  # More conservative boost
            ...
            if len(new_weights) > 0:
                ...
                if use_market_impact:
                    portfolio_value = 1e7  # Assume $10M portfolio
                ...
                # T+1: Set pending weights and cost for next day
...
def bootstrap_sharpe_test(returns: pd.Series, n_bootstrap: int = 10000) -> Dict:
    """Bootstrap test for Sharpe significance. H0: Sharpe <= 0"""
    n = len(returns)
...
def bootstrap_alpha_test(port_returns: pd.Series, bench_returns: pd.Series, n_bootstrap: int = 10000) -> Dict:
    """Bootstrap test for alpha significance. H0: Alpha <= 0"""
    common = port_returns.index.intersection(bench_returns.index)
    port_returns = port_returns.loc[common]
    bench_returns = bench_returns.loc[common]
...
def apply_survivorship_adjustment(metrics: Dict, years: float) -> Dict:
    """
    Apply conservative adjustment for survivorship bias.
    Based on academic research: 1-3% annual alpha overestimation.
    """
    adjustment_per_year = 0.015  # 1.5% per year (conservative)
...
def main():
    ...
    parser.add_argument('--output-dir', default='artifacts/v41_institutional')
    args = parser.parse_args()

    Path(args.output_dir).mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print("v4.1 INSTITUTIONAL-GRADE ALPHA SYSTEM")
    print("=" * 80)
    print(f"Period: {args.start} to {args.end}")
    print(f"Model: {args.model}")
    print(f"Expert Debate: {'OFF' if args.no_debate else 'ON (5 experts)'}")
    ...
    # Deflated Sharpe with proper n_trials
    skew = port_returns.skew()
    kurt = port_returns.kurtosis() + 3
    n_trials = 20  # More conservative: accounts for factor combinations, mode configs, etc.
    euler = 0.5772156649
    ...
    print("\n" + "=" * 80)
    print("RESULTS - v4.1 INSTITUTIONAL-GRADE ALPHA SYSTEM")
    print("=" * 80)

    print(f"\n{'PERFORMANCE (Raw)':^80}")
    print("-" * 80)
    print(f"{'Sharpe Ratio':<40} {metrics['sharpe']:.3f}")
    print(f"{'Alpha (vs SPY)':<40} {metrics['alpha']:+.2%}")
    print(f"{'Annualized Return':<40} {metrics['ann_return']:.2%}")
    print(f"{'Annualized Volatility':<40} {metrics['ann_vol']:.2%}")
    print(f"{'SPY Return (same period)':<40} {metrics['bench_return']:.2%}")
    ...
    checks = [
        metrics['sharpe'] > 1.0,
        metrics['alpha_adjusted'] > 0,  # Use adjusted alpha
        deflated > 0,
        metrics['max_dd'] < 0.20,
        sharpe_test['p_value'] < 0.001,  # Very strict
    ...
    print(f"\n*** INSTITUTIONAL GRADE: {grade} ({passed}/6 checks passed) ***")

    print("\n" + "=" * 80)
    print("LIMITATIONS & DISCLOSURES")
    print("-" * 80)
    print("  1. Survivorship Bias: Uses current stock universe (adjustment applied)")
    print("  2. Data Source: Yahoo Finance (may have look-ahead in splits)")
    print("  3. Market Impact: Simplified Almgren-Chriss model")
    print("  4. No fundamental PIT: Uses price data only")
    print("  5. Parameters: Not optimized on test period")
    print("=" * 80)

    result = {
        'version': 'v4.1-institutional',
        ...
        'limitations': [
            'Survivorship bias (adjustment applied)',
            'Yahoo Finance data',
            'Simplified market impact model',
            'Price data only (no fundamental PIT)',
        ],
    }

    with open(Path(args.output_dir) / 'result_v41.json', 'w') as f:
        json.dump(result, f, indent=2, default=str)

    print(f"\nResults saved to: {args.output_dir}/result_v41.json")
